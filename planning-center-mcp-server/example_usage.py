...
async def example_usage():
    ...
    try:
        ...
        print("Please set PCO_ACCESS_TOKEN or both PCO_APP_ID and PCO_SECRET environment variables")
        return
    ...
    async with PCOClient(config) as client:
        ...
        try:
            ...
            for person in people.data[:3]:  # Show first 3
                print(f"  - {person.attributes.get('first_name', '')} {person.attributes.get('last_name', '')} (ID: {person.id})")
        ...
        try:
            ...
            for service in services.data[:3]:  # Show first 3
                print(f"  - {service.attributes.get('name', 'Unknown')} (ID: {service.id})")
        except Exception as e:
        ...
        try:
            ...
            for registration in registrations.data[:3]:  # Show first 3
                print(f"  - {registration.attributes.get('name', 'Unknown')} (ID: {registration.id})")
        except Exception as e:
