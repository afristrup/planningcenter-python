...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        if name == "get_people":
            ...
            result = paginate_data(filtered_people, arguments.get("per_page", 25), arguments.get("offset", 0))
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        
        elif name == "get_person":
            person = next((p for p in mock_data["people"] if p["id"] == arguments["person_id"]), None)
            if not person:
                return [TextContent(type="text", text="Person not found")]
            return [TextContent(type="text", text=json.dumps(person, indent=2))]
        
        elif name == "get_services":
            result = paginate_data(mock_data["services"], arguments.get("per_page", 25), arguments.get("offset", 0))
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        
        elif name == "get_service":
            service = next((s for s in mock_data["services"] if s["id"] == arguments["service_id"]), None)
            if not service:
            ...
            result = paginate_data(filtered_plans, arguments.get("per_page", 25), arguments.get("offset", 0))
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        
        elif name == "get_plan":
            plan = next((p for p in mock_data["plans"] if p["id"] == arguments["plan_id"]), None)
            if not plan:
            ...
            result = paginate_data(filtered_registrations, arguments.get("per_page", 25), arguments.get("offset", 0))
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        
        elif name == "get_registration":
            registration = next((r for r in mock_data["registrations"] if r["id"] == arguments["registration_id"]), None)
            if not registration:
            ...
            result = paginate_data(filtered_attendees, arguments.get("per_page", 25), arguments.get("offset", 0))
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        
        elif name == "get_attendee":
            attendee = next((a for a in mock_data["attendees"] if a["id"] == arguments["attendee_id"]), None)
            if not attendee:
            ...
            result = {"message": "Mock data reset successfully", "counts": {k: len(v) for k, v in mock_data.items()}}
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
            ...
            result = {
                ...
                "endpoints": [
                    ...
                    "get_attendees", "get_attendee", "reset_mock_data", "get_mock_status"
                ]
            }
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
