...
async def mock_get_person(request: GetPersonRequest) -> Dict[str, Any]:
    ...
    person = next(
        (p for p in mock_data["people"] if p["id"] == request.person_id), None
    )
    if not person:
...
async def mock_get_services(request: GetServicesRequest) -> Dict[str, Any]:
    ...
    return paginate_data(
        mock_data["services"], request.per_page or 25, request.offset or 0
    )
...
async def mock_get_service(request: GetServiceRequest) -> Dict[str, Any]:
    ...
    service = next(
        (s for s in mock_data["services"] if s["id"] == request.service_id), None
    )
    if not service:
...
async def mock_get_registrations(request: GetRegistrationsRequest) -> Dict[str, Any]:
    ...
    return paginate_data(
        filtered_registrations, request.per_page or 25, request.offset or 0
    )
...
async def mock_get_registration(request: GetRegistrationRequest) -> Dict[str, Any]:
    ...
    registration = next(
        (r for r in mock_data["registrations"] if r["id"] == request.registration_id),
        None,
    )
    if not registration:
...
async def mock_get_attendees(request: GetAttendeesRequest) -> Dict[str, Any]:
    ...
    return paginate_data(
        filtered_attendees, request.per_page or 25, request.offset or 0
    )
...
async def mock_get_attendee(request: GetAttendeeRequest) -> Dict[str, Any]:
    ...
    attendee = next(
        (a for a in mock_data["attendees"] if a["id"] == request.attendee_id), None
    )
    if not attendee:
