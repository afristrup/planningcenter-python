        ...
        if name == "get_people":
            ...
            async with pco_client:
                result = await pco_client.get_attendees(
                    ...
                    filter_params={
                        k: v for k, v in arguments.items() 
                        if k in ["event_id", "registration_instance_id", "attendance_status"] and v is not None
                    } or None
                )
                return [TextContent(type="text", text=result.model_dump_json(indent=2))]
...
async def main():
    ...
    try:
        ...
        logger.error("Please set PCO_ACCESS_TOKEN or both PCO_APP_ID and PCO_SECRET environment variables")
        sys.exit(1)
