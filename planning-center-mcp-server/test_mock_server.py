    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                f"{base_url}/get_people",
                json={"per_page": 5, "search": "john"}
            )
            if response.status_code == 200:
                data = response.json()
                print(f"✓ Found {len(data['data'])} people matching 'john'")
                for person in data['data'][:3]:
                    print(f"  - {person['attributes']['full_name']} ({person['attributes']['email']})")
            else:
        ...
        try:
            response = await client.post(
                f"{base_url}/get_registrations",
                json={"per_page": 3, "status": "open"}
            )
            if response.status_code == 200:
                data = response.json()
                print(f"✓ Found {len(data['data'])} open registrations")
                for registration in data['data']:
                    print(f"  - {registration['attributes']['name']} ({registration['attributes']['status']})")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for attendee in data['data']:
                    print(f"  - {attendee['attributes']['first_name']} {attendee['attributes']['last_name']} ({attendee['attributes']['attendance_status']})")
            else:
        ...
        try:
            response = await client.post(
                f"{base_url}/get_people",
                json={"per_page": 2, "offset": 0}
            )
            if response.status_code == 200:
                data = response.json()
                print(f"✓ Pagination working - page 1: {len(data['data'])} items")
                print(f"  Total: {data['meta']['total']}, Offset: {data['meta']['offset']}")
                
                # Test next page
                if data['links']['next']:
                    next_response = await client.post(
                        f"{base_url}/get_people",
                        json={"per_page": 2, "offset": 2}
                    )
        ...
        try:
            response = await client.post(
                f"{base_url}/get_people",
                json={"per_page": 10, "status": "active"}
            )
            if response.status_code == 200:
                data = response.json()
                active_count = len([p for p in data['data'] if p['attributes']['status'] == 'active'])
